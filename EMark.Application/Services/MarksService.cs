...
namespace EMark.Application.Services
{
    public class MarksService : IMarksService
    {
        ...
        public async Task CreateMarkColumn(MarkColumnModel model)
        {
            ...
            var subject = await _databaseContext.Subjects.AsNoTracking().SingleOrDefaultAsync(subject => subject.Id == model.SubjectId && teacherGroups.Contains(subject.GroupId));
            if (subject is null)
            {
            ...
            bool isMarkColumnAlreadyExist = await _databaseContext.MarkColumns.AnyAsync(markColumn => markColumn.Name == model.Name && markColumn.SubjectId == model.SubjectId);
        ...
        public async Task CreateMark(MarkModel model)
        {

            var teacherId = int.Parse(_jwtTokenReader.UserId);
            var teacherGroups = _databaseContext.Groups.Where(group => group.TeacherGroups.Any(teacher => teacher.Id == teacherId)).Select(group => group.Id).ToArray();

            var markColumn = await _databaseContext.MarkColumns.AsNoTracking().SingleOrDefaultAsync(markColumn => markColumn.Id == model.MarkColumnId && teacherGroups.Contains(markColumn.Subject.GroupId));
            if (markColumn is null)
            {
                throw new NotFoundException("MarkColumn is not found");
            }

            var student = await _databaseContext.Users.OfType<Student>().AsNoTracking().SingleOrDefaultAsync(student => student.Id == model.StudentId);
            if (student is null)
            {
            ...
            {
                Value = model.Value,
                MarkColumnId = model.MarkColumnId,
